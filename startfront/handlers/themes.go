...
func HandleThemesGet(db *gorm.DB, data json.RawMessage, c *gin.Context) {
	var req struct {
		ID   uint   `json:"id"`
	...
	var m models.Themes
	query := db.Model(&models.Themes{}).Preload("Client").Where("id = ?", req.ID)
	if req.Name != "" {
...
func HandleThemesList(db *gorm.DB, data json.RawMessage, c *gin.Context) {
	var req struct {
		Page   *int    `json:"page"`
	...
	q := db.Model(&models.Themes{}).Preload("Client")
...
func HandleThemesInsert(db *gorm.DB, data json.RawMessage, c *gin.Context) {
	var req models.Themes
	if err := json.Unmarshal(data, &req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"code": "1", "status": "error", "error": "invalid request data: " + err.Error()})
		return
	}
	...
	// Duplicate check (name + scope + client_id)
	var existing models.Themes
	if err := db.Where(
		"name = ? AND scope = ? AND COALESCE(client_id, 0) = COALESCE(?, 0)",
		req.Name, req.Scope, req.ClientID,
	).First(&existing).Error; err == nil {
		c.JSON(http.StatusConflict, gin.H{"code": "1", "status": "error", "error": "duplicate theme"})
		return
	}

	// Default variables if nil
	if len(req.Variables) == 0 {
		req.Variables = datatypes.JSON([]byte(`{}`))
	}
...
func HandleThemesUpdate(db *gorm.DB, data json.RawMessage, c *gin.Context) {
	var req struct {
		ID        uint            `json:"id"`
		Name      *string         `json:"name"`
		Scope     *string         `json:"scope"`
	...
	if len(req.Variables) > 0 {
		m.Variables = datatypes.JSON(req.Variables)
	}
	if req.IsActive != nil {
		m.IsActive = *req.IsActive
	}

	// Validate scope rules again
	switch m.Scope {
	case "global":
		if m.ClientID != nil {
			c.JSON(http.StatusBadRequest, gin.H{"code": "1", "status": "error", "error": "global scope cannot have client_id"})
...
func HandleThemesDelete(db *gorm.DB, data json.RawMessage, c *gin.Context) {
	var req struct {
		ID uint `json:"id"`
	}
	...
	res := db.Delete(&models.Themes{}, req.ID)
	if res.Error != nil {
	...
	c.JSON(http.StatusOK, gin.H{"code": "0", "status": "Success", "message": "theme deleted"})
}
