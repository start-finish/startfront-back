...
func HandleClientsGet(db *gorm.DB, data json.RawMessage, c *gin.Context) {
	var req struct {
		ID     uint   `json:"id"`
	...
	var m models.Clients
	query := db.Model(&models.Clients{}).Where("id = ?", req.ID)
...
func HandleClientsList(db *gorm.DB, data json.RawMessage, c *gin.Context) {
	var req struct {
		Page   *int    `json:"page"`
...
func HandleClientsInsert(db *gorm.DB, data json.RawMessage, c *gin.Context) {
	var req models.Clients
	if err := json.Unmarshal(data, &req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"code": "1", "status": "error", "error": "invalid request data: " + err.Error()})
		return
	}
...
func HandleClientsUpdate(db *gorm.DB, data json.RawMessage, c *gin.Context) {
	var req struct {
		ID     uint   `json:"id"`
		Name   string `json:"name"`
		Status string `json:"status"`
...
func HandleClientsDelete(db *gorm.DB, data json.RawMessage, c *gin.Context) {
	var req struct {
		ID uint `json:"id"`
	}
	if err := json.Unmarshal(data, &req); err != nil {
	...
	res := db.Delete(&models.Clients{}, req.ID)
	if res.Error != nil {
